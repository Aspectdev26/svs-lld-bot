...
const TROLL_LINES = [
  ...
  "when are you going to leave the kids alone and build a cold sorc?",
];
...
/** Same roll, fired on any message the target posts in any guild channel (including private match channels); replies in-place. */
export async function maybeTrollMessage(message: Message): Promise<void> {
  if (message.author.id !== TROLL_TARGET_USER_ID) return;
  ...
  const sent = await message.reply({ content: line });
  scheduleMessageCleanup(sent, TROLL_MESSAGE_CLEANUP_MS);
}
